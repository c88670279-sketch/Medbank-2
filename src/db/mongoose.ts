...
export async function normalizeAndRebuildCurriculum() {
  try {
    ...
    const uniquePairs: { subject: string; chapter: string }[] = [];
    const collectPairs = async (Model: any) => {
    ...
    for (const q of allAnatomyQuestions) {
      ...
      if (mappedPart) {
        ...
        if (!q.topic || q.topic.trim() === '') q.topic = 'General';
      }
      await q.save();
    }
    ...
    for (const model of modelsToMigrate) {
      ...
      for (const item of items) {
        ...
        if (mappedPart) {
          ...
          if (!item.topic || item.topic.trim() === '') item.topic = 'General';
        }
        await item.save();
      }
    }
    ...
    const collectTopics = async (Model: any) => {
