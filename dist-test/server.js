...
async function ensureSubjectAndChapter(subjectName, chapterName) {
    ...
    try {
        ...
        if (!subjectObj) {
            ...
            if (lower.includes('patho'))
                matchedStandardName = 'Pathology';
            else if (lower.includes('phar'))
                matchedStandardName = 'Pharmacology';
            else if (lower.includes('micro'))
                matchedStandardName = 'Microbiology';
            else if (lower.includes('anat'))
                matchedStandardName = 'Anatomy';
            else if (lower.includes('physio'))
                matchedStandardName = 'Physiology';
            else if (lower.includes('biochem'))
                matchedStandardName = 'Biochemistry';
            else if (lower.includes('surg'))
                matchedStandardName = 'Surgery';
            else if (lower.includes('pedia'))
                matchedStandardName = 'Pediatrics';
            else if (lower.includes('obg') || lower.includes('gyne') || lower.includes('obstet'))
                matchedStandardName = 'Obstetrics & Gynecology';
            else if (lower.includes('ophthal'))
                matchedStandardName = 'Ophthalmology';
            else if (lower.includes('ent') || lower.includes('oto'))
                matchedStandardName = 'ENT';
            else if (lower.includes('foren') || lower.includes('fmt'))
                matchedStandardName = 'Forensic Medicine';
            else if (lower.includes('comm') || lower.includes('psm') || lower.includes('preventive'))
                matchedStandardName = 'Community Medicine';
            else if (lower.includes('derma'))
                matchedStandardName = 'Dermatology';
            else if (lower.includes('psych'))
                matchedStandardName = 'Psychiatry';
            else if (lower.includes('radio'))
                matchedStandardName = 'Radiology';
            else if (lower.includes('anes'))
                matchedStandardName = 'Anesthesia';
            else if (lower.includes('ortho'))
                matchedStandardName = 'Orthopedics';
            else if (lower.includes('med'))
                matchedStandardName = 'Medicine';
            ...
            if (!subjectObj) {
                ...
                if (lowerStandard.includes('patho'))
                    icon = 'ShieldAlert';
                else if (lowerStandard.includes('phar'))
                    icon = 'Droplet';
                else if (lowerStandard.includes('micro'))
                    icon = 'Activity';
                else if (lowerStandard.includes('anat'))
                    icon = 'Layers';
                else if (lowerStandard.includes('physio'))
                    icon = 'Heart';
                else if (lowerStandard.includes('biochem'))
                    icon = 'Atom';
                else if (lowerStandard.includes('med'))
                    icon = 'Stethoscope';
                else if (lowerStandard.includes('surg'))
                    icon = 'Scissors';
                else if (lowerStandard.includes('pedia'))
                    icon = 'Baby';
                else if (lowerStandard.includes('ophthal'))
                    icon = 'Eye';
                else if (lowerStandard.includes('foren'))
                    icon = 'FileText';
                else if (lowerStandard.includes('comm'))
                    icon = 'Users';
...
app.use((req, res, next) => {
    ...
    const vercelForwardedPath = req.headers['x-vercel-forwarded-path'] || req.headers['x-forwarded-path'];
...
function handleGeminiError(err, res, contextDescription) {
...
function parseMCQsFromTextFallback(text, defaultSubject, defaultChapter, defaultTopic) {
...
function validateAndScoreQuestion(q) {
    const errors = [];
    let score = 100;
...
app.post('/api/gemini/import-mcqs', async (req, res) => {
    ...
    try {
        ...
        const subjectListStr = dbSubjectsList.length > 0
            ? dbSubjectsList.map((s) => `'${s.name}'`).join(', ')
...
app.put('/api/subjects/:id', async (req, res) => {
    try {
        const subject = await SubjectModel.findOneAndUpdate({ id: req.params.id }, { $set: req.body }, { new: true });
        if (!subject) {
...
app.put('/api/chapters/:id', async (req, res) => {
    try {
        ...
        if (data.subject !== undefined)
            updatedFields.subject = data.subject;
        if (data.description !== undefined)
            updatedFields.description = data.description;
        if (data.displayOrder !== undefined)
            updatedFields.displayOrder = Number(data.displayOrder);
        if (data.isActive !== undefined)
            updatedFields.isActive = !!data.isActive;
        const updatedChapter = await ChapterModel.findOneAndUpdate({ id: req.params.id }, { $set: updatedFields }, { new: true });
...
app.put('/api/topics/:id', async (req, res) => {
    try {
        ...
        if (data.chapterId !== undefined)
            updatedFields.chapterId = data.chapterId;
        if (data.subject !== undefined)
            updatedFields.subject = data.subject;
        if (data.description !== undefined)
            updatedFields.description = data.description;
        const updatedTopic = await TopicModel.findOneAndUpdate({ id: req.params.id }, { $set: updatedFields }, { new: true });
...
app.post('/api/curriculum/move', async (req, res) => {
    try {
        ...
        if (type === 'chapter') {
            ...
            if (!targetSubject)
                return res.status(404).json({ success: false, error: 'Target Subject not found.' });
        ...
        if (type === 'topic') {
            ...
            if (!targetChapter)
                return res.status(404).json({ success: false, error: 'Target Chapter not found.' });
    ...
    try {
        ...
        if (type === 'subject') {
            ...
            if (!sourceSub || !targetSub)
                return res.status(404).json({ success: false, error: 'Source or Target subject not found.' });
        ...
        if (type === 'chapter') {
            ...
            if (!sourceChap || !targetChap)
                return res.status(404).json({ success: false, error: 'Source or Target chapter not found.' });
        ...
        if (type === 'topic') {
            ...
            if (!sourceTop || !targetTop)
                return res.status(404).json({ success: false, error: 'Source or Target topic not found.' });
...
app.put('/api/pdfs/:id', async (req, res) => {
    try {
        const pdf = await PdfModel.findOneAndUpdate({ id: req.params.id }, { $set: req.body }, { new: true });
        if (!pdf) {
...
app.put('/api/notes/:id', async (req, res) => {
    try {
        const note = await NoteModel.findOneAndUpdate({ id: req.params.id }, { $set: req.body }, { new: true });
        if (!note) {
...
// ================= QUESTIONS API =================
const getQuestionFindQuery = (val) => {
    const conds = [{ id: val }];
...
app.get('/api/questions/counts', async (req, res) => {
    try {
        ...
        subjectStats.forEach((item) => {
            if (item._id)
                subjects[item._id] = item.count;
        });
        chapterStats.forEach((item) => {
            if (item._id)
                chapters[item._id] = item.count;
        });
        topicStats.forEach((item) => {
            if (item._id)
                topics[item._id] = item.count;
        });
        res.json({
            success: true,
            total,
...
app.put('/api/questions/:id', async (req, res) => {
    try {
        ...
        const question = await QuestionModel.findOneAndUpdate(getQuestionFindQuery(req.params.id), { $set: req.body }, { new: true });
        if (!question) {
...
app.put('/api/tests/:id', async (req, res) => {
    try {
        const test = await TestModel.findOneAndUpdate({ id: req.params.id }, { $set: req.body }, { new: true });
        if (!test) {
...
app.get('/api/testResults', async (req, res) => {
    try {
        ...
        let results;
        if (email) {
            results = await TestResultModel.find({ email: email.toLowerCase() }).sort({ date: -1 });
        }
        else {
...
app.put('/api/testResults/:id', async (req, res) => {
    try {
        const result = await TestResultModel.findOneAndUpdate({ id: req.params.id }, { $set: req.body }, { new: true });
        if (!result) {
...
app.put('/api/announcements/:id', async (req, res) => {
    try {
        const announcement = await AnnouncementModel.findOneAndUpdate({ id: req.params.id }, { $set: req.body }, { new: true });
        if (!announcement) {
...
app.put('/api/flashcards/:id', async (req, res) => {
    try {
        const flashcard = await FlashcardModel.findOneAndUpdate({ id: req.params.id }, { $set: req.body }, { new: true });
        if (!flashcard) {
...
app.put('/api/videos/:id', async (req, res) => {
    try {
        const video = await VideoModel.findOneAndUpdate({ id: req.params.id }, { $set: req.body }, { new: true });
        if (!video) {
