...
const t = 'className="w-full px-3 py-2 bg-slate-50 dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-xl text-xs focus:outline-none focus:ring-1 focus:ring-emerald-500 text-slate-800 dark:text-slate-100"\n              />\n            </div>\n            <div className="pt-2 border-t border-slate-100 dark:border-zinc-900 space-y-2">';

const r = `className="w-full px-3 py-2 bg-slate-50 dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-xl text-xs focus:outline-none focus:ring-1 focus:ring-emerald-500 text-slate-800 dark:text-slate-100"
              />
            </div>
...
if (index !== -1) {
    code = code.substring(0, index) + r + code.substring(index + t.length);
    fs.writeFileSync(file, code);
    console.log("Success");
} else {
    // try removing carriage returns
    const t2 = t.replace(/\r/g, '');
    let index2 = code.replace(/\r/g, '').indexOf(t2);
    if (index2 !== -1) {
        console.log("Found without CR");
        let startCode = code.replace(/\r/g, '').substring(0, index2);
        let endCode = code.replace(/\r/g, '').substring(index2 + t2.length);
        fs.writeFileSync(file, startCode + r + endCode);
    } else {
